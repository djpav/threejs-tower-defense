 ...
 * Smooths a polyline by replacing sharp corners with rounded curves.
 * Preserves first and last points (spawn/goal positions).
 */
export function chaikinSmooth(points: WorldPosition[], iterations = 2): WorldPosition[] {
  if (points.length <= 2) return points;

  let result = points;
  for (let k = 0; k < iterations; k++) {
    const newPts: WorldPosition[] = [result[0]]; // preserve start
    ...
    newPts.push(result[result.length - 1]); // preserve end
    result = newPts;
  }
  return result;
}
...
/**
 * Apply subtle noise displacement to smoothed waypoints for a handcrafted look.
 * Offsets are perpendicular to the path direction at each point.
 * First and last points are preserved (spawn/goal positions).
 * Uses a simple seeded hash for deterministic noise.
 */
export function applyPathNoise(
  ...
  const result: WorldPosition[] = [points[0]]; // preserve start
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    // Deterministic noise from index + seed (Knuth hash)
    const hash = (((i * 2654435761 + seed * 1597334677) >>> 0) / 4294967296) * 2 - 1;
    const offset = hash * amplitude;
  ...
  result.push(points[points.length - 1]); // preserve end
  return result;
}
