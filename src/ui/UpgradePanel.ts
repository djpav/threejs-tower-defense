...
import { SELL_REFUND_RATIO } from "@/configs/GameBalanceConfigs";
...
const PRIORITY_LABELS: Record<TargetingPriority, string> = {
  ...
  [TargetingPriority.Threat]: "Threat",
};
...
export class UpgradePanel extends UIComponent {
  ...
  constructor(onUpgrade: () => void, onSell: () => void, onClose: () => void) {
    ...
    this.root.className =
      "fixed bottom-[52px] left-1/2 -translate-x-1/2 hidden items-center gap-4 max-sm:gap-2 px-5 max-sm:px-3 py-3 max-sm:py-2 panel-glass border-2 border-game-gold text-white font-game text-sm max-sm:text-xs pointer-events-auto z-[11] select-none max-w-[95vw] flex-wrap max-sm:justify-center";
  ...
  showTower(info: TowerInfo, gold: number): void {
    ...
    const refund = Math.floor(info.totalInvested * SELL_REFUND_RATIO);
