...
export class SpeedControls extends UIComponent {
  ...
  private btnCleanups: Array<() => void> = [];
  ...
  constructor(callbacks: SpeedControlsCallbacks) {
    ...
    this.root.className =
      "fixed top-12 right-3 max-sm:top-11 max-sm:right-2 flex gap-1 pointer-events-auto z-[12]";
    ...
    // Mute button (part of the control strip)
  ...
  private createButton(label: string, onClick: () => void): HTMLButtonElement {
    ...
    this.btnCleanups.push(() => btn.removeEventListener("click", onClick));
    return btn;
  }
  ...
  dispose(): void {
    for (const cleanup of this.btnCleanups) cleanup();
    this.btnCleanups = [];
