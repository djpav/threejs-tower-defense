...
export class MessageOverlay extends UIComponent {
  ...
  constructor() {
    super("div");
    this.root.className = "message-overlay max-sm:px-4";
  }
  ...
  showLevelComplete(
    ...
    title.className = "text-[64px] max-sm:text-[36px] text-center";
    ...
    starEl.className = "text-5xl max-sm:text-3xl tracking-[8px] max-sm:tracking-[4px]";
    for (let i = 0; i < 3; i++) {
    ...
    statsEl.className = "text-[22px] max-sm:text-[16px] font-normal text-game-gray-lighter text-center leading-[1.8]";
    statsEl.innerHTML = `
